...
describe('createNullProjectCustomField', () => {
  describe('NULL safety', () => {
    ...
    it('should not throw if `maxLength` param is not defined', () => {
      expect(() =>
        createNullProjectCustomField('projectName', 'name', undefined),
      ).not.toThrow();
    });
    it('should not throw if `name` param is not defined', () => {
      expect(() =>
        createNullProjectCustomField('projectName', 'label', undefined),
      ).not.toThrow();
    });
  });
  describe('Field', () => {
    ...
    it('should return project custom field with a project name not longer than provided length', () => {
      expect(
        ...
        projectCustomField: {
          field: {
            name: projectLabelMock,
          },
        },
        value: {
          name: getProjectName(10),
        },
      });
    });
    it('should return project custom field with a project name', () => {
      ...
      expect(
        ...
        projectCustomField: {
          field: {
            name: projectCustomFieldNameMock,
          },
        },
        value: {
          name: getProjectName(10),
    ...
    function getProjectName(maxLength = nullProjectCustomFieldMaxLength) {
      return projectNameMock.length > maxLength
        ? `${projectNameMock.slice(0, maxLength - 3)}…`
        : projectNameMock;
    }
  });
});
