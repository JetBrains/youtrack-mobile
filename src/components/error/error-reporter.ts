...
export const getAppAndDeviceData = (): string => {
  ...
  try {
    config = (getStorageState().config as any) as AppConfig;
  } catch (e) {
...
export async function createReportErrorData(
  error: Error | string,
  isCrashReport: boolean = false,
): Promise<ReportErrorData> {
