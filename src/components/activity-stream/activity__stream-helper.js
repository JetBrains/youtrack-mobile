...
function updateActivityCommentReactions({
  ...
  currentUser,
  reaction,
}: { comment: IssueComment, currentUser: User, reaction: Reaction }) {
  const _comment: IssueComment = {...comment};
  _comment.reactions = _comment?.reactions || [];
  _comment.reactionOrder = _comment.reactionOrder || '';

  const existedCommentReaction: ?Reaction = _comment.reactions.find(
    (it: Reaction) => it.reaction === reaction.reaction && it?.author?.id === currentUser?.id
  );

  if (existedCommentReaction) {
    _comment.reactions = _comment.reactions.filter((it: Reaction) => it?.id !== existedCommentReaction.id && existedCommentReaction.author.id === currentUser.id);
    const anotherUserHasReaction: boolean = _comment.reactions.some((it: Reaction) => it.reaction === reaction.reaction);
    if (!anotherUserHasReaction) {
      _comment.reactionOrder = (
        _comment.reactionOrder
    ...
    const containsReaction: boolean = _comment.reactionOrder.split(COMMENT_REACTIONS_SEPARATOR).some(
      (reactionName: string) => reactionName === reaction.reaction
    );
    if (!containsReaction) {
      _comment.reactionOrder = [_comment.reactionOrder, reaction.reaction].filter(Boolean).join('|');
    }
  }
