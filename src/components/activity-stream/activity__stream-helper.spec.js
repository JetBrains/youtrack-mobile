...
describe('Stream Helper', () => {
  ...
  describe('updateActivityCommentReactions', () => {
    ...
    beforeEach(() => {
      ...
      reactionMock = {reaction: 'heart', id: 'heart', author: userMock};
    ...
    it('should add a new reaction to a comment that has no reactions', () => {
      const updatedComment = updateActivityCommentReactions(
        {comment: commentMock, currentUser: userMock, reaction: reactionMock}
      );
    ...
    it('should add a new reaction to a comment that has reactions', () => {
      const commentReactionsMock = [{reaction: anotherReactionName}];
      commentMock = mocks.createCommentMock({reactions: commentReactionsMock, reactionOrder: anotherReactionName});

      const updatedComment = updateActivityCommentReactions(
        {comment: commentMock, currentUser: userMock, reaction: reactionMock}
      );

      expect(updatedComment.reactions).toEqual(commentReactionsMock.concat(reactionMock));
      expect(updatedComment.reactionOrder).toEqual(`${anotherReactionName}|${reactionMock.reaction}`);
    });
    ...
    it('should remove an existed user reaction', () => {
      ...
      commentMock.reactionOrder = [reactionMock.reaction, anotherReactionName].join('|');
    ...
    function createReaction(name = '', author = mocks.createUserMock()) {
      return {id: guid(), reaction: name, author};
    }
  });
});
