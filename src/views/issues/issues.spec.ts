...
describe('Issue list actions', () => {
  ...
  it('should set issues query', () => {
    actions.setIssuesQuery(TEST_QUERY).should.deep.equal({
  ...
  describe('Suggestions', () => {
    ...
    it('should use legacy REST endpoint', async () => {
      ...
      ).toHaveBeenCalledWith(stateMock.issueList.query, 2);
    });
    it('should use latest REST endpoint and with the search context', async () => {
      ...
      ).toHaveBeenCalledWith(stateMock.issueList.query, 5, [searchContextMock]);
    });
    ...
    async function doSuggest(caretPosition: number = 4) {
      await actions.suggestIssuesQuery(TEST_QUERY, caretPosition)(
        dispatch,
        getState,
        () => apiMock,
      );
  ...
  it('should clear query assist suggestions', () => {
    actions.clearAssistSuggestions().should.deep.equal({
  ...
  describe('loadIssuesCount', () => {
    ...
    it('should load issues count', async () => {
      ...
      await actions.loadIssuesCount(issueContextQueryMock, folderMock)(
        dispatch,
        () => stateMock,
        () => apiMock,
      );
