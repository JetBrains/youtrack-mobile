...
const SettingsAppearance = ({onHide, backIcon}: {onHide: () => void; backIcon?: void}) => {
  const renderThemeCheckbox = (currentTheme: Theme, uiTheme: Record<string, any>) => {
    const userThemeMode: string | null | undefined = getStorageState().themeMode || '';
    const mode: string = uiTheme.mode;
    const isChecked =
      (!userThemeMode && uiTheme.system) || (!uiTheme.system && !!userThemeMode && userThemeMode.indexOf(mode) !== -1);
    ...
    return (
      ...
      >
        <View style={styles.settingsListItemOption}>
          ...
          {isChecked && <IconCheck size={24} color={currentTheme.uiTheme.colors.$link} />}
        </View>
      </TouchableOpacity>
    );
  };
      ...
      {(theme: Theme) => (
        <View style={styles.settings}>
          <Header
            ...
            leftButton={backIcon || <IconBack color={theme.uiTheme.colors.$link} />}
            onBack={onHide}
          />

          <View style={styles.settingsAppearance}>
            {[systemTheme].concat(themes).map((it: Record<string, any>) => renderThemeCheckbox(theme, it))}
          </View>
        </View>
      )}
    </ThemeContext.Consumer>
