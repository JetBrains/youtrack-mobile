...
describe('Inbox Threads List', () => {
  ...
  describe('Render', () => {
    ...
    it('should render error', async () => {
      createStore({inboxThreads: {error: new Error()}});
      const {getByTestId} = doRender();
    ...
    it('should render progress placeholder', async () => {
      createStore({inboxThreads: {inProgress: true}});
      const {getByTestId} = doRender();
    ...
    describe('`No notifications message` render', () => {
      ...
      it('should not render the message if there is no any messages in threads', async () => {
        prepareStore([mocks.createThreadMock({messages: []})]);
        const {getByTestId} = doRender();
      ...
      it('should show text about no unread notifications', async () => {
        prepareStore([]);
        mocks.setStorage({inboxThreadsCache: {unreadOnly: true}});
        const {queryByTestId} = doRender();

        expect(queryByTestId('test:id/inboxThreadsListEmptyMessageText')).toHaveTextContent('You don’t have any unread notifications');
      });

      it('should show text about no notifications', async () => {
        prepareStore([]);
        mocks.setStorage({inboxThreadsCache: {unreadOnly: false}});
        const {queryByTestId} = doRender();

        expect(queryByTestId('test:id/inboxThreadsListEmptyMessageText')).toHaveTextContent('No notifications');
      });

      function prepareStore(threads, inProgress = false, error) {
        createStore({
          inboxThreads: {
            threadsData: {[folderIdAllKey]: {threads}},
            inProgress,
            error,
          },
        });
  ...
  function createInboxThreadsData(key = folderIdAllKey, threads = [], hasMore = false) {
    return {inboxThreads: {threadsData: {[key]: {threads, hasMore}}}};
  }
