...
export const createDispatchActions = (
  ...
  setIssueId: (issueId: string): { issueId: string, type: any } => {
    return actions.SET_ISSUE_ID({issueId});
  },
  startIssueRefreshing: (): { type: any } => {
    return actions.START_ISSUE_REFRESHING();
  },
  stopIssueRefreshing: (): { type: any } => {
    return actions.START_ISSUE_REFRESHING();
  },
  receiveIssue: (issue: IssueFull): { issue: IssueFull, type: any } => {
    return actions.RECEIVE_ISSUE({issue});
  },
  setIssueFieldValue: (field: CustomField, value: FieldValue): { field: CustomField, type: any, value: FieldValue } => {
    return actions.SET_ISSUE_FIELD_VALUE({field, value});
  },
  setProject: (project: IssueProject): { project: IssueProject, type: any } => {
    return actions.SET_PROJECT({project});
  },
  startEditingIssue: (): { type: any } => {
    return actions.START_EDITING_ISSUE();
  },
  stopEditingIssue: (): { type: any } => {
    return actions.STOP_EDITING_ISSUE();
  },
  setIssueSummaryAndDescription: (
    summary: string,
    description: string
  ): { description: string, summary: string, type: any } => {
    return actions.SET_ISSUE_SUMMARY_AND_DESCRIPTION({summary, description});
  },
  setIssueSummaryCopy: (summary: string): { summary: string, type: any } => {
    return actions.SET_ISSUE_SUMMARY_COPY({summary});
  },
  setIssueDescriptionCopy: (description: string): { description: string, type: any } => {
    return actions.SET_ISSUE_DESCRIPTION_COPY({description});
  },
  startSavingEditedIssue: (): { type: any } => {
    return actions.START_SAVING_EDITED_ISSUE();
  },
  stopSavingEditedIssue: (): { type: any } => {
    return actions.STOP_SAVING_EDITED_ISSUE();
  },
  setVoted: (voted: boolean): { type: any, voted: boolean } => {
    return actions.SET_VOTED({voted});
  },
  setStarred: (starred: boolean): { starred: boolean, type: any } => {
    return actions.SET_STARRED({starred});
  },
  issueUpdated: (issue: IssueFull): { issue: IssueFull, type: any } => {
    return actions.RECEIVE_ISSUE({issue});
  },
  unloadActiveIssueView: (): { type: any } => {
    return actions.UNLOAD_ACTIVE_ISSUE_VIEW();
  },
  openTagsSelect: (selectProps: any) => {
    return actions.OPEN_ISSUE_SELECT({selectProps});
  },
  closeTagsSelect: (): { selectProps: null, isTagsSelectVisible: boolean } => {
    return actions.CLOSE_ISSUE_SELECT({selectProps: null});
  },
  setError(error: CustomError) {
    return actions.RECEIVE_ISSUE_ERROR({error});
  },
  openCommandDialog: (initialCommand: string = ''): { initialCommand: string, type: any } => {
    return {type: commandDialogTypes.OPEN_COMMAND_DIALOG, initialCommand};
  },
  closeCommandDialog: (): { type: any } => {
    return {type: commandDialogTypes.CLOSE_COMMAND_DIALOG};
  },
  receiveCommandSuggestions: (suggestions: CommandSuggestionResponse): { suggestions: CommandSuggestionResponse, type: any } => {
    return {type: commandDialogTypes.RECEIVE_COMMAND_SUGGESTIONS, suggestions};
  },
  startApplyingCommand: (): { type: any } => {
    return {type: commandDialogTypes.START_APPLYING_COMMAND};
  },
  stopApplyingCommand: (): { type: any } => {
    return {type: commandDialogTypes.STOP_APPLYING_COMMAND};
  },
  receiveIssueVisibility: (visibility: Visibility): { type: any, visibility: Visibility } => {
    return actions.RECEIVE_ISSUE_VISIBILITY({visibility});
  },
  receiveAllAttachments: (attachments: Array<Attachment>): {type: any, attachments: Array<Attachment>} => {
    return {type: attachmentTypes.ATTACH_RECEIVE_ALL_ATTACHMENTS, attachments};
  },
  receiveIssueLinks: (links: Array<IssueOnList>) => {
    return actions.RECEIVE_ISSUE_LINKS({links});
  },
